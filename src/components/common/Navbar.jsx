...
function Navbar(props) {
  ...
  return (
    <div
      className={`hidden justify-center xl:flex xl:flex-col h-fit pl-8 py-5 w-fit md:w-72 fixed z-20 lg:border-r-2 xl:border-gray-600 gap-6`}
    >
      <>
        <Link to='/'>
          <div className={`fontLogo h-[4rem] text-5xl hover:font-bold shadow-xl `}>Friendify</div>
        </Link>
      </>
      {/* Friendify logo done */}
      <>
        <Link to='/'>
          <div className={`mt-5 flex gap-3 mb-1  text-2xl`} to="upload"><AiFillHome className="text-2xl" /> Home</div>
        </Link>
      </>

      <>
        <Link to='#'>
          <div className={`flex text-base gap-3 mb-1 `} onClick={ () => setShowSearch(!showSearch)
      ...
      {showSearch ? (
        ...
        <form onSubmit={searchHandler}>
          ...
          <div className="relative">
            ...
            <input type="search" id="default-search" className="block w-full p-4 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark: dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="..Search" onChange={changeHandler} required name="search" />
      ...
      <>
        <Link to='/'>
          <div className={`flex text-base gap-3 mb-1 `}><MdOutlineTravelExplore className="text-2xl" /> Explore</div>
        </Link>
      </>

      <>
        <Link to='/'>
          <div className={`flex text-base gap-3 mb-1 `}><IoAddCircleSharp className="text-2xl" /> Create</div>
        </Link>
      </>

      <>
        <Link to='/'>
          <div className={`flex text-base gap-3 mb-1 `}><BsFillHeartFill className="text-2xl" /> Like</div>
        </Link>
      </>

      <>
        <Link to={`/profile/${id}`}>
          <div className={`flex text-base gap-3 mb-1 `}><BiUserCircle className="text-2xl" />Profile { }</div>
        </Link>
      </>

      <>
          
            <div className={`cursor-pointer font-semibold hover:font-bold  flex gap-3 text-lg`} 
              onClick={async () => {
